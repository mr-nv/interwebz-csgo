...
void DrawHeadBox(CBaseEntity* pEnt, float &boxz)
{
	...
	mstudiobbox_t *pBox = pStudioHeader->pHitboxSet(0)->pHitbox(0);
...
void CESP::draw()
{
	...
	for (INT ax = 1; ax <= g_Valve.pEngine->GetMaxClients(); ax++)
	{
		...
		if(g_CVARS.CvarList[VisChecks])
			bVis = g_Aimbot.IsVisible(pMe->GetEyePosition(),vOrigin,0x46004003,(ValveSDK::CTrace::ITraceFilter*)&g_Aimbot.tfNoPlayers);
		...
		Vector vTopScreen;
		if( !g_Valve.WorldToScreen( vTop, vTopScreen ) ) continue;
