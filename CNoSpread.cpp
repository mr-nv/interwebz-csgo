...
void CNoSpread::GetRollSpreadFix(ValveSDK::CBaseCombatWeapon *m_pWeapon, UINT seed, Vector& pflInAngles)
{
	...
	float flA = RandomFloat( 0.0f, m_pWeapon->GetCone( ) );
	float flB = RandomFloat( 0.0f, 6.283185f );
	float flC = RandomFloat( 0.0f, m_pWeapon->GetSpread( ) );
	float flD = RandomFloat( 0.0f, 6.283185f );
